...
var Board = function (options) {
  ...
  //RF
  this.rfId = options && options.rfId || "000"; //Code to identify the main board for RF com
    ...
    if (err) {
      ...
      setTimeout(function(){
        ...
        if (self.debug) {
          ...
          self.write('99' + self.normalizePin(0) + self.normalizePin(1));
          process.on('SIGINT', function(){
            self.log('info', 'sending debug mode toggle off to board');
            self.write('99' + self.normalizePin(0) + self.normalizePin(0));
            delete self.serial;
            setTimeout(function(){
              process.exit();
            }, 100);
...
Board.prototype.log = function (/*level, message*/) {
  ...
  if (this.debug) {
    console.log(String(+new Date()).grey + ' duino '.blue + chalk.magenta(args.shift()) + ' ' + args.join(', '));
  }
}
 ...
 * Send RF raw msg
 */
Board.prototype.sendRfMsg = function (receiver, msg) {
  var msg = '9300' + this.rfId + receiver + msg;
  this.write(msg);
}
 ...
 * val == in  = 000
 */
Board.prototype.rfPinMode = function (receiver, pin, val) {
  pin = this.normalizePin(pin);
  val = (
    val == 'out' ?
    this.normalizeVal(1) :
  ...
  var msg = '00' + pin + val;
  this.sendRfMsg(receiver, msg);
}

/*
 * Send RF digitalWrite
 */
Board.prototype.rfDigitalWrite= function (receiver, pin, val) {
 ...
 * Send RF analogWrite
 */
Board.prototype.rfAnalogWrite= function (receiver, pin, val) {
